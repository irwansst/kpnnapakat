...
switch($_GET[act]){
  ...
  default:
	...
	$no=1;
	$tampil	= mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM smasuk WHERE  periode='$_SESSION[periode]' AND nArsip IS NULL ORDER BY stamp DESC");
...
// BATAS UNTUK SCRIPT UPLOAD FILE			
  ...
  case "tambahsMasuk":
    ...
    echo "	
			...
			$nom = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nAgenda from smasuk ORDER BY id_sMasuk DESC LIMIT 1");
  ...
  case "editsMasuk":
    $edit = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM sMasuk WHERE id_sMasuk='$_GET[id]'");
