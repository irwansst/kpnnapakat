...
switch($_GET[act]){
  ...
  default:
    ...
    echo "<h1>DAFTAR KEGIATAN $_SESSION[periode]</h1>";
    
	echo "<input type=button class='button' value='Tambah Data' onclick=location.href='?op=kegiatan&act=tambahkegiatan'><br><br>";
     ...
     while ($r=mysql_fetch_array($tampil)){
		...
		if (empty($opt) AND $sd > 3) {
			...
			echo "<td align=center><img src='images/FINISHED_0.png'></td>";
			 ...
			 echo "
			<a href=?op=kegiatan&act=outputkegiatan&id=$r[id]><img class='masterTooltip'  src='images/report.png' border='0' title='Rekam Output'></a>
			<a href=?op=kegiatan&act=editkegiatan&id=$r[id]><img class='masterTooltip'  src='images/edit.png' border='0' title='Ubah Kegiatan'></a>
			<a href=?op=kegiatan&act=lihatkegiatan&id=$r[id]><img class='masterTooltip'  src='images/show.png' border='0' title='Lihat' Kegiatan'></a>
			";
  ...
  case "tambahkegiatan":
      echo "<h1>Tambah Kegiatan</h1>
			<div class='line'></div>
			<div class='box'>
			";
    echo "	
	        <form method=POST action='$aksi?op=kegiatan&act=input'>
			<label><span>KEGIATAN</span><input type='text' name='vkegiatan' size='70'></label>
  ...
  case "editkegiatan":
	...
	echo "<h1>Ubah Kegiatan</h1>
			...
			<div class='box'>";
    echo "
        <form method=POST action=$aksi?op=kegiatan&act=update>
        <input type=hidden name=id value=$r[id]>
		
		<label><span>KEGIATAN</span><input type='text' name='vkegiatan' value='$r[kegiatan]' size='70'></label>
	...
	echo "<h1>Rekam Output</h1>
			...
			<div class='box'>";
    echo "
        <form method=POST action=$aksi?op=kegiatan&act=output>
        <input type=hidden name=id value=$r[id]>
		<label><span>OUTPUT KEGIATAN</span><input type='text' name='voutput' value='$r[output]' size='70'></label>
		<input type=submit class=button value=Simpan>
        <input type=button class=button value=Batal onclick=self.history.back()></td></tr>
        </form></div>";
    break;
...
case "uploadFile":
	...
	echo "<h1>Upload Berkas File</h1>
			<div class='line'></div>
			<div class='demo-jui'>
			<form method='post' enctype='multipart/form-data' action='$aksi?op=kegiatan&act=upload'>
